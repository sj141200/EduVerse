...
function InstructorProfilePage(){
  ...
  async function onDelete(){
    if (!confirm('Delete your instructor account? This will delete all your courses and related content.')) return
    setDeleting(true)
    try {
      const res = await deleteUserAccount(token)
      if (res && res.success) {
        showMessage('Account deleted and related courses removed', 'success')
        logout()
        navigate('/')
      } else {
